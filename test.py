...
def test_acted_together_tiny():
    with open("resources/tiny.pickle", "rb") as f:
        smalldb = pickle.load(f)
    test_data = lab.transform_data(smalldb)
    actor1 = 4724
    actor2 = 2876
    assert lab.acted_together(test_data, actor1, actor2) is True
...
def test_bacon_number_tiny_0():
    with open("resources/tiny.pickle", "rb") as f:
        smalldb = pickle.load(f)
    data = lab.transform_data(smalldb)
    #How many actors have bacon number of n?
    assert lab.actors_with_bacon_number(data, 0) == {4724}

def test_bacon_number_tiny_1():
    with open("resources/tiny.pickle", "rb") as f:
        smalldb = pickle.load(f)
    data = lab.transform_data(smalldb)
    #How many actors have bacon number of n?
    assert lab.actors_with_bacon_number(data, 1) == {2876, 1532}

def test_bacon_number_tiny_2():
    with open("resources/tiny.pickle", "rb") as f:
        smalldb = pickle.load(f)
    data = lab.transform_data(smalldb)
    #How many actors have bacon number of n?
    assert lab.actors_with_bacon_number(data, 2) == {1640}

def test_bacon_number_tiny_3():
    with open("resources/tiny.pickle", "rb") as f:
        smalldb = pickle.load(f)
    data = lab.transform_data(smalldb)
    #How many actors have bacon number of n?
    assert lab.actors_with_bacon_number(data, 3) == set()
...
def test_bacon_path_tiny():
    with open("resources/tiny.pickle", "rb") as f:
        smalldb = pickle.load(f)
    data = lab.transform_data(smalldb)
    assert lab.bacon_path(data, 1640) == [4724, 2876, 1640]
